      ...
      <div className="max-w-6xl mx-auto px-6 py-6">
        ...
        {isAdmin ? (
          filteredPayments.length > 0 ? (
            <AdminTableView payments={filteredPayments} />
          ) : (
            <EmptyState />
          )
        ) : (
  ...
  <div className="bg-white rounded-2xl shadow-md overflow-x-auto">
    <table className="min-w-full text-sm">
      ...
      <tbody className="divide-y divide-gray-100">
        {payments.map((ticket, idx) => (
          <tr key={ticket._id} className="hover:bg-indigo-50/40 transition-colors">
            <td className="px-4 py-3 text-gray-400">{idx + 1}</td>
            <td className="px-4 py-3 font-medium text-gray-800">{ticket.Title || 'N/A'}</td>
            <td className="px-4 py-3 text-gray-700">{ticket.name}</td>
            <td className="px-4 py-3 text-gray-500">{ticket.email || ticket.userId?.email || '—'}</td>
            <td className="px-4 py-3">
              <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${typeColor(ticket.ticketType)}`}>
                {(ticket.ticketType || 'N/A').toUpperCase()}
              </span>
            </td>
            <td className="px-4 py-3 text-center">{ticket.ticketCount}</td>
            <td className="px-4 py-3 font-bold text-indigo-600">${ticket.totalAmount}</td>
            <td className="px-4 py-3 text-gray-600">{ticket.paymentMethod}</td>
            <td className="px-4 py-3">
              <span className={`text-xs font-semibold px-2 py-1 rounded-full ${statusColor(ticket.status)}`}>
                {(ticket.status || 'N/A').toUpperCase()}
              </span>
            </td>
          </tr>
