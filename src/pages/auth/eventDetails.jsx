...
import { useRateEventMutation, useCheckBookingQuery } from '../../features/ApplicationApi';
...
const EventDetails = () => {
  ...
  const { data: bookingData } = useCheckBookingQuery(event?._id, { skip: !isNormalUser || !event?._id });
  const hasBooked = bookingData?.hasBooked === true;
      ...
      <div className="max-w-6xl mx-auto rounded-2xl overflow-hidden relative" style={{ zIndex: 1, background: 'rgba(15, 15, 40, 0.85)', border: '1px solid rgba(139, 92, 246, 0.3)', backdropFilter: 'blur(10px)', boxShadow: '0 0 40px rgba(139, 92, 246, 0.2)' }}>
        ...
        <div className="p-8">
          ...
          <div className="pt-6 mb-6" style={{ borderTop: '1px solid rgba(139, 92, 246, 0.3)' }}>
            ...
            {/* Submit Rating - only for users who booked this event */}
            {isNormalUser && (
              hasBooked ? (
                submitted || userRating ? (
                  <p className="text-green-400 font-semibold">✓ You rated this event {userRating?.rating || selected} ★</p>
                ) : (
                ...
                <div className="p-4 rounded-xl flex items-center gap-3" style={{ background: 'rgba(30, 20, 60, 0.6)', border: '1px solid rgba(139, 92, 246, 0.3)' }}>
                  <svg className="w-5 h-5 text-purple-400 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/>
                  </svg>
                  <p className="text-purple-300 text-sm">Only users who have booked this event can rate it.</p>
                </div>
              )
            )}
          </div>
