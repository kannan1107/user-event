...
const Analytics = () => {
    ...
    const totalIncome = tickets.reduce((s, t) => s + (t.totalAmount || 0), 0);
    ...
    const eventsByCategoryData = useMemo(() => {
        ...
        return { labels: Object.keys(map), datasets: [{ label: 'Events', data: Object.values(map), backgroundColor: COLORS, borderColor: BORDERS, borderWidth: 1 }] };
    }, [events]);

    const ticketsPerEventData = useMemo(() => {
        const map = {};
        tickets.forEach(t => { const title = t.Title || 'Unknown'; map[title] = (map[title] || 0) + (t.ticketCount || 0); });
        return { labels: Object.keys(map), datasets: [{ label: 'Tickets Sold', data: Object.values(map), backgroundColor: COLORS, borderColor: BORDERS, borderWidth: 1 }] };
    }, [tickets]);
    ...
    const bookingsOverTimeData = useMemo(() => {
        ...
        return { labels: sorted, datasets: [{ label: 'Bookings', data: sorted.map(k => map[k]), borderColor: 'rgba(99,102,241,1)', backgroundColor: 'rgba(99,102,241,0.15)', fill: true, tension: 0.4, pointRadius: 4 }] };
    }, [tickets]);

    const incomePerEventData = useMemo(() => {
        const map = {};
        tickets.forEach(t => { const title = t.Title || 'Unknown'; map[title] = (map[title] || 0) + (t.totalAmount || 0); });
        return { labels: Object.keys(map), datasets: [{ label: 'Income ($)', data: Object.values(map), backgroundColor: COLORS, borderColor: BORDERS, borderWidth: 1 }] };
    }, [tickets]);

    const revenueByOrganizerData = useMemo(() => {
        const map = {};
        tickets.forEach(t => { const org = t.eventId?.organizer || 'Unknown'; map[org] = (map[org] || 0) + (t.totalAmount || 0); });
        return { labels: Object.keys(map), datasets: [{ data: Object.values(map), backgroundColor: COLORS, borderColor: BORDERS, borderWidth: 2 }] };
    }, [tickets]);

    const myTicketsPerEventData = useMemo(() => {
        const map = {};
        tickets.forEach(t => { const title = t.Title || 'Unknown'; map[title] = (map[title] || 0) + (t.ticketCount || 0); });
        return { labels: Object.keys(map), datasets: [{ label: 'My Tickets', data: Object.values(map), backgroundColor: COLORS, borderColor: BORDERS, borderWidth: 1 }] };
    }, [tickets]);
    ...
    const mySpendingOverTimeData = useMemo(() => {
        ...
        const sorted = Object.keys(map).sort((a, b) => new Date(a) - new Date(b));
        return { labels: sorted, datasets: [{ label: 'Amount Spent ($)', data: sorted.map(k => map[k]), borderColor: 'rgba(16,185,129,1)', backgroundColor: 'rgba(16,185,129,0.15)', fill: true, tension: 0.4, pointRadius: 4 }] };
    }, [tickets]);
    ...
    if (!isAdmin) {
        ...
        const myTotalSpent = tickets.reduce((s, t) => s + (t.totalAmount || 0), 0);
        ...
        return (
            <div className="p-6 max-w-5xl mx-auto min-h-screen bg-gray-50">
                ...
                <div className="grid grid-cols-3 gap-4 mb-8">
                    <StatCard label="My Bookings" value={tickets.length} color="border-indigo-500" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <ChartCard title="My Ticket Type Breakdown">
                        {myTicketTypeData.labels.length > 0
                            ? <div className="flex justify-center"><div style={{ maxWidth: 300, width: '100%' }}><Chart type="doughnut" data={myTicketTypeData} options={{ responsive: true, plugins: { legend: { position: 'bottom' } } }} /></div></div>
                            : <EmptyState />}
                    </ChartCard>
                </div>
            </div>
        ...
        <div className="p-6 max-w-7xl mx-auto min-h-screen bg-gray-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <ChartCard title="Events by Category">
                    {eventsByCategoryData.labels.length > 0 ? <Chart type="bar" data={eventsByCategoryData} options={baseOpts()} /> : <EmptyState />}
                </ChartCard>
                <ChartCard title="Tickets Sold per Event">
                    {ticketsPerEventData.labels.length > 0 ? <Chart type="bar" data={ticketsPerEventData} options={{ ...baseOpts(), indexAxis: 'y', plugins: { legend: { display: false } } }} /> : <EmptyState />}
                </ChartCard>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <ChartCard title="Ticket Bookings Over Time">
                    {bookingsOverTimeData.labels.length > 0 ? <Chart type="line" data={bookingsOverTimeData} options={baseOpts()} /> : <EmptyState />}
                </ChartCard>
                <ChartCard title="Total Income per Event">
                    {incomePerEventData.labels.length > 0 ? <Chart type="bar" data={incomePerEventData} options={{ ...baseOpts(), plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { callback: v => `$${v}` } } } }} /> : <EmptyState />}
                </ChartCard>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ChartCard title="Revenue by Organizer">
                    {revenueByOrganizerData.labels.length > 0
                        ? <div className="flex justify-center"><div style={{ maxWidth: 320, width: '100%' }}><Chart type="doughnut" data={revenueByOrganizerData} options={{ responsive: true, plugins: { legend: { position: 'bottom' }, tooltip: { callbacks: { label: ctx => ` $${ctx.parsed.toFixed(2)}` } } } }} /></div></div>
                        : <EmptyState />}
                </ChartCard>
                <ChartCard title="User Role Distribution">
                    {users.length > 0 ? (() => {
                        ...
                        const data = { labels: Object.keys(map).map(r => r.charAt(0).toUpperCase() + r.slice(1)), datasets: [{ data: Object.values(map), backgroundColor: COLORS, borderColor: BORDERS, borderWidth: 2 }] };
                        return <div className="flex justify-center"><div style={{ maxWidth: 320, width: '100%' }}><Chart type="pie" data={data} options={{ responsive: true, plugins: { legend: { position: 'bottom' } } }} /></div></div>;
                    })() : <EmptyState />}
                </ChartCard>
            </div>
        </div>
